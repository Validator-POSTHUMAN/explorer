...
<template>
  <div class="bg-black rounded-lg text-sm py-2">
    <table class="table-compact w-full table-fixed hidden lg:!table">
      <tbody class="gap-4">
        <tr v-for="(item, index) in proposals?.proposals" :key="index">
          <div class="border border-[#686868] rounded-lg flex">
            <td class="px-4 w-20">
              <label
                for="proposal-detail-modal"
                class="text-[#3FB6A8] text-base hover:text-gray-400 cursor-pointer"
            ...
            <td class="w-1/3">
              <div>
                <RouterLink
                  ...
                  class="text-[#3FB6A8] text-base mb-1 block hover:text-gray-400 truncate"
                >
                  {{
                    item?.content?.title ||
                    item?.title ||
                ...
                <div
                  v-if="item.content"
                  class="bg-[#1C1C21] text-[#03C6F2] inline-block rounded-full px-2 py-[1px] text-xs mb-1"
                >
            ...
            <td v-if="statusMap?.[item?.status] === 'VOTING'" class="w-3/3">
              <div>
                <label
                  for="vote"
    ...
    <div class="lg:!hidden">
      ...
      >
        <div
          class="text-[#3FB6A8] text-base mb-1 flex justify-between hover:text-gray-400"
        >
          <RouterLink
            :to="`/${chain.chainName}/gov/${item?.proposal_id}`"
