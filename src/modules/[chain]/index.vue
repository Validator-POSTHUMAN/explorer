...
import {
  ...
  useTxDialog,
  useWalletStore,
  useStakingStore, // delete after checking?
  useParamStore,
} from '@/stores';
...
const props = defineProps(['chain']); // delete after checking?
...
const stakingStore = useStakingStore(); // delete after checking?
...
<template>
  <div class="flex grow">
    <div
      ...
      class="flex flex-col gap-3 rounded grow shadow mt-1.5 mx-0 md:mx-3"
    >
      <!-- Top block -->
      <div class="grid grid-cols-2 xl:grid-cols-12 gap-3.5">
        <!-- Coin info -->
        <div
          class="col-span-2 xl:col-span-9 thick-border-block px-3 md:px-[30px] pt-5 pb-8"
        >
          <div
            class="flex justify-center md:justify-between items-center mb-[50px] gap-6"
          >
            <!-- address -->
            <AddressWithCopy
              :href="`/${chain}/account/${walletStore.currentAddress}`"
            ...
            <div class="grid grid-cols-3 gap-4">
              ...
              <label
                for="send"
                class="btn-outline"
                @click="dialog.open('send', {}, walletStore.loadMyAsset)"
                >{{ $t('account.btn_send') }}</label
              >
              <label
                for="delegate"
                class="btn-outline"
                @click="dialog.open('delegate', {}, walletStore.loadMyAsset)"
          ...
          <div
            class="grid grid-cols-1 md:!grid-cols-2 xl:!grid-cols-4 auto-cols-auto gap-2.5"
          >
            <div
              v-for="item in coinStatisticsInfo"
              class="flex border rounded border-[#222226] bg-black/50"
              :key="item.title"
            >
              <div
                class="flex justify-center items-center bg-[#0D110E] rounded text-button-text h-full px-2"
              >
                <Icon
                  v-if="item.icon"
                  :icon="item.icon"
              ...
              <div class="grow px-2.5 pt-2 pb-4">
                <p
                  class="uppercase header-14-medium-aa tracking-wide text-header-text mb-2"
                >
                  {{ $t(item.title) }}
                </p>
        ...
        >
          <h3
            class="header-20-medium-aa text-header-text tracking-wide uppercase mb-2 px-3"
          >
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              class="hidden md:block ml-1.5 text-sm capitalize text-[#54829F]"
              >{{ item?.name }}</span
            >
          </a>
        </div>
        ...
        <div class="col-span-1 md:col-span-3 xl:col-span-2">
          <div
            class="flex md:flex-col justify-between items-center md:items-start mb-4 gap-1"
          >
            ...
            <div class="text-sm text-addition">
              {{ $t('index.rank') }}:
              <span
                class="bg-[#33583B] dark:bg-[#33583B] text-green-400 text-[10px] py-0.5 px-3 rounded-full"
              >
          ...
          <div>
            ...
            <div class="flex">
              ...
              <a
                class="header-16-medium my-2.5 py-1 grow bg-green-button hover:bg-green-button-hover rounded-full flex justify-center uppercase"
                :class="{
