...
const supply = computed(
  () => store.stats.find((item) => item.title === 'Supply')?.stats
);
const bonded_tokens = computed(
  () => store.stats.find((item) => item.title === 'Bonded Tokens')?.stats
);
const apr = computed(
  () =>
    (Number(mintStore.inflation) *
      100 *
      Number(supply.value?.replace(/,/g, ''))) /
    Number(bonded_tokens.value?.replace(/,/g, ''))
);
...
const widgetsInfo = computed(() => [
  ...
  },
  {
    ...
    icon: IconLock,
    numValue: +getMonthProgressPercent(
      +formatSeconds(staking.params?.unbonding_time).replace(/days/, '')
    ),
...
const calcAmount = (
  amount: Coin | Coin[] | undefined,
  type: 'value' | 'token'
) => {
  if (!amount) return '-';
  return format
    .formatToken(returnAmounItem(amount), true, '0,0.[000000]')
    .split(' ')[type === 'value' ? 0 : 1];
};

const listForTable = computed(() =>
  list.value.map((item, key) => ({
    ...
    validator: {
      ...
      address:
        item.v.description?.website || item.v.description?.identity || '-',
    ...
    <div class="md:sticky top-0">
      ...
      >
        <div
          ...
          v-for="widget in widgetsInfo"
          :key="widget.name"
        >
          <CircleProgressComponent
            class="hidden md:block"
            :value="widget.numValue"
          >
            <component :is="widget.icon" />
          </CircleProgressComponent>

          <div
            class="w-full thin-border-block items-center md:items-start md:-ml-5 md:pl-8"
          >
            <CircleProgressComponent
            ...
            <p
              class="header-14-medium-aa text-header-text tracking-wide truncate uppercase"
            >
              {{ $t(widget.name) }}
            </p>
            <p class="header-20-medium text-white tracking-wide truncate">
    ...
    <div class="w-full">
      <div class="flex items-center justify-between py-1">
        <div class="w-full tabs tabs-boxed bg-transparent gap-2.5">
          <a
            v-for="item in tabs"
            class="btn-fill w-full md:w-36"
            :class="{ 'bg-button-v2': tab === item.name }"
            @click="tab = item.name"
            >{{ $t(item.value) }}</a
          >
        </div>
            ...
            <table class="table bg-black/20 staking-table">
                ...
                <tr class="text-header-text body-text-14">
                  ...
                  <td scope="col" class="text-right">
                    <div class="flex gap-1 items-center justify-end">
                      ...
                      <SortButton
                        @click="() => toggleSortDirection('voting_power_value')"
                      />
                    </div>
                  </td>
                  <td scope="col" class="text-right">
                  ...
                  <td scope="col" class="flex justify-end items-center gap-1">
                    ...
                    <SortButton
                      @click="() => toggleSortDirection('commission_value')"
                    />
                  </td>
                  <td scope="col" class="text-right">
                    <span class="pr-5">
              ...
              <tbody class="">
                ...
                >
                  ...
                  <td>
                    <div
                      class="body-text-14 text-button-text truncate relative px-2 py-1 rounded-full w-fit"
                  ...
                  <td>
                    <div
                      class="max-w-[300px] flex items-center overflow-hidden"
                    >
                      <div class="flex flex-col">
                        <span
                          class="max-w-[300px] text-button-text header-16-medium uppercase dark:invert truncate"
                        >
                          <RouterLink
                  ...
                  <td class="text-right">
                    <div class="flex flex-col">
                      <h6
                        class="header-14-medium-aa tracking-wide whitespace-nowrap text-white"
                      >
                        <span>{{ voting_power.value }}</span>
                  ...
                  <!-- 👉 24h Changes -->
                  <td
                    class="text-right body-text-14"
                    :class="change24Color(changes)"
                  >
                  ...
                  <td class="text-right">
                    <div class="flex justify-end">
                      ...
                      <label
                        v-else
                        for="delegate"
                        class="w-36 btn-outline border-button-text"
                        @click="
